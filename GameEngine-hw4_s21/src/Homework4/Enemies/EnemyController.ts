...
export default class EnemyController extends StateMachineAI {
	...
	initializeAI(owner: GameNode, options: Record<string, any>){
		...
		if(options.name == "spikeball"){
			this.player = options.player;
			this.spikeball = true;
			jump = new SpikeJump(this, owner);
		}
	...
	playerNearby(): boolean{
		var firstpow = Math.pow(this.player.position.x - this.owner.position.x ,2);
		var secondpow = Math.pow(this.player.position.y - this.owner.position.y , 2);
	...
	update(deltaT: number): void {
		if(this.spikeball && this.player != null){
